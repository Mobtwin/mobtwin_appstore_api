...
class Proxy {
    ...
    addActiveOne = (proxy) => {
      ...
      this.activeProxies.map((px) => {
        if (px.id === proxy.id) isExist = true;
      });
      !isExist ? this.activeProxies.push(proxy) : null;
    };
    removeActiveOne = (proxy) => {
        this.activeProxies = this.activeProxies.filter((px) => px.id != proxy.id);
    };
    ...
    removeInActiveOne = (proxy) => {
        this.inActiveProxies = this.inActiveProxies.filter((px) => px.id != proxy.id);
    };
